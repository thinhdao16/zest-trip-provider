...
function ScreenMain() {
  ...
  const handleCreateRefundForCus = () => {
    ...
    dispatch(createRefundForCus(data));
  };
  return (
                ...
                {filteredData[0]?.TicketOnBooking?.map(
                  (ticketQuantity: any, index: number) => (
                    <div key={index} className=" grid grid-cols-4 ">
                      ...
                      <span className="  text-center">
                        {formatNumber(parseInt(ticketQuantity?.original_price))}
                      </span>{" "}
          ...
          {moreRefund && (
            <div className="mt-4 bg-white w-1/2 border border-solid border-gray-300 rounded-lg relative">
              ...
              <div className="px-6 py-4">
                {filteredData[0]?.status === "ACCEPTED" && (
                  <p className="font-medium text-xl">Create Refund</p>
                )}
                {filteredData[0]?.status === "USER_REQUEST_REFUND" && (
                  <p className="font-medium text-xl"> Accept Refund</p>
                )}
              </div>
              <hr />
              <div className="p-6">
                        ...
                        {selectedImages.length < 1 ? (
                          ...
                          >
                            ...
                            >
                              ...
                              <div>Choose image you want to create</div>
                            </div>
